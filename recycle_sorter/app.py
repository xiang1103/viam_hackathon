...
from .policy.piles import Piles
...
async def run_sort(robot: LiveRobot, mode: str, max_picks: int = 50) -> Counter:
    """Empty the unsorted zone into sorted piles, re-surveying after every pick.

    survey -> segment -> choose -> classify -> pile for that class -> pick -> place in its next slot
    """
    sort_cfg = load_yaml("sort.yaml")
    workspace = robot.manip.workspace
    piles = Piles(robot.manip.poses, workspace)
    piles.validate()
    classifier, policy = make_classifier(mode, sort_cfg), SortPolicy(sort_cfg, mode, piles.names)
    ...
    for _ in range(max_picks):
        ...
        pile = policy.pile_for(result)
        log.info("target %s (%.2f) at (%.0f, %.0f) -> %s", result.label, result.confidence, *target.centroid, pile)
        ...
        try:
            ...
            await place(robot.manip, *piles.next_slot(pile))
        ...
        failures = 0
        sorted_counts[pile] += 1

    log.info("piles: %s", policy.legend)
...
async def run_replay(frames_dir: Path, mode: str) -> None:
    ...
    for d in dirs:
        ...
        overlay = save_debug(frame, objects, results, f"replay-{d.name}", workspace)
        ...
        for o, r in zip(objects, results):
            ...
            print(
                ...
                f"size={o.width:.0f}x{o.length:.0f}x{o.height:.0f} yaw={o.yaw_deg:.0f} -> {policy.pile_for(r)}"
            )
    print(f"\npiles: {policy.legend}")
