...
- `pipeline.py` is a front end on top of `recycle_sorter`. For each command:
  ...
  5. Files: only `data/scans/<launch time>/`, made once per launch, holding the latest look's two annotated YOLO pictures, `survey.png` and `scan.png`. Each look overwrites them. `order.json` there gets one entry per command: the parsed order, `not_available`, what was fetched (or `planned` in a dry run) and what was `missing`. An ordered category that no can was read as is skipped and reported as missing; nothing else on the table is touched (`tests/test_pipeline.py` runs this end to end with fakes).
